"""services.case_store - the case event log + projections, as a service.

Extracted from the retired ``workers/investigate.py`` NOTIFY-mirror pipeline
so the investigator AGENT can write its own events directly. The agent (or
any caller) appends events and projections through these functions; the
merchant UI keeps reading the exact same tables (events / findings / cases /
actions) with zero changes.

Surface:
    append_event(...)               - atomic-seq event append (5-retry race guard)
    record_finding_projection(...)  - findings-table insert w/ citation normalization
    record_brief(...)               - cases.decision_* + drafted-actions materialization
    finalize_case(...)              - status transitions + policy auto-approval
    update_status(...)              - bare cases.status update
    serialize(...)                  - JSON-friendly serializer for agent payloads

All functions acquire connections from the global pool (manthan_api.db
.get_pool), matching the worker they were extracted from. Behavior is
preserved exactly - this module is a move, not a rewrite.
"""
...
logger = logging.getLogger("services.case_store")
...
async def append_event(
    ...
    *,
    ...
    ``trace_id`` / ``span_id`` are accepted for forward compatibility but
    currently IGNORED: the events table (schema/001_initial.sql) has no
    trace columns and we deliberately do not alter the schema here.
    """
    del trace_id, span_id  # no trace/span columns on events yet
...
async def record_finding_projection(
    org_id: UUID,
    case_id: UUID,
    data: dict[str, Any],
) -> None:
    """Project a finding_recorded event payload into the findings table
    so the UI doesn't have to scan events on every read."""
...
# ──────────────────────────────────────────────────────────────────────
# Brief projection (decision + drafted actions)
# ──────────────────────────────────────────────────────────────────────
...
async def record_brief(
    ...
    """Capture the decision on the case row so the UI shows the verdict,
    AND materialize the drafted_actions into the actions table for the
    Action Executor to drain on approval."""
    if not isinstance(data, dict):
        return
    ...
    if not drafted:
        drafted = await _synthesize_actions(
    ...
    if isinstance(drafted, list) and drafted:
        drafted = await _enrich_drafted_actions(
            org_id=org_id, thread_id=thread_id, case_id=case_id,
            decision_action=action, amount_minor=amount,
            decision_rationale=(
...
# ──────────────────────────────────────────────────────────────────────
# Finalization (status transitions + policy auto-approval)
# ──────────────────────────────────────────────────────────────────────
...
async def finalize_case(
    ...
    *,
    ...
    Args mirror what the old worker derived from its agent_store:
      has_brief - a brief_drafted event exists for this case
      hitl      - a hitl_pause event exists (case escalates to a human)
      errored   - the run terminated with reason == "error"

    Returns the final status written to the cases row.
    """
    log = logger.getChild(str(thread_id)[:8] + ".finalize")
    final_status = "awaiting_approval"  # default until HITL exists
    if hitl:
        final_status = "escalated"
    if not has_brief and errored:
    ...
    if final_status == "awaiting_approval" and has_brief:
        try:
...
async def update_status(case_id: UUID, status: str) -> None:
    """Bare cases.status update (e.g. 'investigating', 'errored')."""
    async with get_pool().acquire() as conn:
        await conn.execute(
...
# ──────────────────────────────────────────────────────────────────────
# Drafted-action synthesis + enrichment (moved verbatim from the worker)
# ──────────────────────────────────────────────────────────────────────
...
async def _synthesize_actions(
    ...
    if customer and re.match(r"^[^@\s]+@[^@\s]+\.[^@\s]+$", str(customer)):
...
async def _enrich_drafted_actions(
    ...
    if not charge_id:
        m = re.search(r"Duplicate charge:\s*(ch_[A-Za-z0-9]+)", trigger_text)
        if m:
            charge_id = m.group(1)
        else:
            m = re.search(r"(ch_[A-Za-z0-9]+)", trigger_text)
    ...
    if not dispute_id or not str(dispute_id).startswith("du_"):
        m = re.search(r"(du_[A-Za-z0-9]+)", trigger_text)
        if m:
    ...
    if not customer_email and customer_ref and re.match(
...
# ──────────────────────────────────────────────────────────────────────
# Pure helpers (moved verbatim from the worker)
# ──────────────────────────────────────────────────────────────────────
